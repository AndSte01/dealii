...
int main(int argc, char **argv)
{
  ...
  for (unsigned int i = 0; i < quadrature_formula_linear.size(); ++i)
    std::cout << quadrature_formula_linear.point(i) << " "
              << quadrature_formula_linear.weight(i) << std::endl;

  std::cout << std::endl;
  for (unsigned int i = 0; i < quadrature_formula_quadratic.size(); ++i)
    std::cout << quadrature_formula_quadratic.point(i) << " "
              << quadrature_formula_quadratic.weight(i) << std::endl;

  try
    {
      for (unsigned int degree = 1; degree <= 3; ++degree)
        {
          std::cout << "Solving with Pyramid elements fo degree " << degree
                    << std::endl
                    << "============================================="
                    << std::endl
                    << std::endl;
